...
\node[eqcaption] at ($(box1.south) + (0,\eqyshift)$)
    {$h^{(1)}_v = \sigma\!\left(\mathbf{W}^{(1)}_{\text{self}}\, x_v + \displaystyle\sum_{u\in\mathcal{N}(v)} \mathbf{W}^{(1)}_{\text{neigh}}\, x_u\right)$};

\node[eqcaption] at ($(box2.south) + (0,\eqyshift)$)
    {$h^{(2)}_v = \sigma\!\left(\mathbf{W}^{(2)}_{\text{self}}\, h^{(1)}_v + \displaystyle\sum_{u\in\mathcal{N}(v)} \mathbf{W}^{(2)}_{\text{neigh}}\, h^{(1)}_u\right)$};
